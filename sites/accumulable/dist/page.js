...
const o=t=>t??T;class DecidablesSlider extends DecidablesElement{static get properties(){return{disabled:{attribute:"disabled",type:Boolean,reflect:!0},max:{attribute:"max",type:Number,reflect:!0},min:{attribute:"min",type:Number,reflect:!0},step:{attribute:"step",type:Number,reflect:!0},value:{attribute:"value",type:Number,reflect:!0}}}constructor(){super(),this.disabled=!1,this.max=void 0,this.min=void 0,this.step=void 0,this.value=void 0}changed(t){this.value=t.target.value,this.dispatchEvent(new CustomEvent("change",{detail:{value:this.value},bubbles:!0}))}inputted(t){this.value=t.target.value}static get styles(){return[super.styles,i$2`
    ...
    `}}customElements.define("ddm-fit",DDMFit);class DDMParameters extends AccumulableElement{static get properties(){return{a:{attribute:"boundary-separation",type:Number,reflect:!0},z:{attribute:"starting-point",type:Number,reflect:!0},v:{attribute:"drift-rate",type:Number,reflect:!0},t0:{attribute:"nondecision-time",type:Number,reflect:!0}}}constructor(){super(),this.a=void 0,this.z=void 0,this.v=void 0,this.t0=void 0}setBoundarySeparation(t){this.a=+t.target.value,this.dispatchEvent(new CustomEvent("ddm-parameters-a",{detail:{a:this.a},bubbles:!0}))}setStartingPoint(t){this.z=+t.target.value,this.dispatchEvent(new CustomEvent("ddm-parameters-z",{detail:{z:this.z},bubbles:!0}))}setDriftRate(t){this.v=+t.target.value,this.dispatchEvent(new CustomEvent("ddm-parameters-v",{detail:{v:this.v},bubbles:!0}))}setNondecisionTime(t){this.t0=+t.target.value,this.dispatchEvent(new CustomEvent("ddm-parameters-t0",{detail:{t0:this.t0},bubbles:!0}))}static get styles(){return[super.styles,i$2`
      ...
      `]}render(){return x$1``}willUpdate(){this.alignState()}update(t){if(super.update(t),Number.isNaN(this.width)||Number.isNaN(this.height)||Number.isNaN(this.rem))return;const e=this.width,r=this.height;let n,i;e/r>1.5?(i=r,n=1.5*i):(n=e,i=n/1.5);const a=1*this.rem,s=3*this.rem,o=3.75*this.rem,l=3.25*this.rem,c=i-(a+s),u=n-(o+l),d=.75*this.rem,h=.5*c,p=.25*c-d,g=.75*this.rem,y=.9*u,m=.1*u-g,b=parseInt(this.getComputedStyleValue("---transition-duration"),10),f=linear().domain([this.scale.time.min,this.scale.time.max]).range([0,y]),$=linear().domain([this.scale.evidence.min,this.scale.evidence.max]).range([h,0]),I=linear().domain([this.scale.density.min,this.scale.density.max]).range([p,0]),C=linear().domain([this.scale.density.min,this.scale.density.max]).range([0,p]),E=linear().domain([0,1]).range([0,c]),B=drag().subject(((t,e)=>({x:f(e.t0),y:$(e.startingPoint)}))).on("start",(t=>{select(t.currentTarget).classed("dragging",!0)})).on("drag",(t=>{this.drag=!0;const e=!!t.sourceEvent.shiftKey&&(Math.abs(t.x-t.subject.x)>Math.abs(t.y-t.subject.y)?"t0":"z");let r=f.invert(t.x),n=($.invert(t.y)+this.a/2)/this.a;r="z"===e?f.invert(t.subject.x):r<0?0:r>500?500:r,n="t0"===e?($.invert(t.subject.y)+this.a/2)/this.a:n<.01?.01:n>.99?.99:n,this.t0=r,this.z=n,this.alignState(),this.dispatchEvent(new CustomEvent("ddm-model-t0",{detail:{t0:this.t0},bubbles:!0})),this.dispatchEvent(new CustomEvent("ddm-model-z",{detail:{z:this.z},bubbles:!0}))})).on("end",(t=>{select(t.currentTarget).classed("dragging",!1),this.drag=!1})),A=drag().on("start",(t=>{select(t.currentTarget).classed("dragging",!0)})).on("drag",(t=>{this.drag=!0;let e=($.invert(t.y)-this.startingPoint)/(f.invert(t.x)-this.t0)*1e3;e=e<.01?.01:e>5?5:e,this.v=e,this.alignState(),this.dispatchEvent(new CustomEvent("ddm-model-v",{detail:{v:this.v},bubbles:!0}))})).on("end",(t=>{select(t.currentTarget).classed("dragging",!1),this.drag=!1})),Q=drag().subject(((t,e)=>({x:0,y:$(e.value)}))).on("start",(t=>{select(t.currentTarget).classed("dragging",!0)})).on("drag",((t,e)=>{this.drag=!0;let r=$.invert(t.y);r=r<this.scale.evidence.min?this.scale.evidence.min:r>this.scale.evidence.max?this.scale.evidence.max:"upper"===e.bound&&r<.005?.005:"lower"===e.bound&&r>-.005?-.005:r,this.a=Math.abs(2*r),this.alignState(),this.dispatchEvent(new CustomEvent("ddm-model-a",{detail:{a:this.a},bubbles:!0}))})).on("end",(t=>{select(t.currentTarget).classed("dragging",!1),this.drag=!1})),v=line().x((t=>f(t.t))).y((t=>$(t.e))),U=line().x((t=>f(t.t))).y((t=>I(t.d))),F=line().x((t=>f(t.t))).y((t=>C(t.d))),R=select(this.renderRoot).selectAll(".main").data([{width:this.width,height:this.height,rem:this.rem}]),x=R.enter().append("svg").classed("main",!0).html(AccumulableElement.svgDefs),S=x.append("defs");S.append("marker").attr("id","measure-arrow").attr("orient","auto-start-reverse").attr("markerUnits","userSpaceOnUse").attr("viewBox","-5 -5 10 10").attr("refX","2").attr("refY","0").attr("markerWidth","10").attr("markerHeight","10").append("path").attr("stroke","context-stroke").attr("fill","context-stroke").attr("d","M -3 -3 l 6 3 l -6 3 z"),S.append("marker").attr("id","model-sd-cap").attr("orient","auto-start-reverse").attr("markerUnits","userSpaceOnUse").attr("viewBox","-5 -5 10 10").attr("refX","0").attr("refY","0").attr("markerWidth","10").attr("markerHeight","10").append("path").attr("stroke","context-stroke").attr("fill","context-stroke").attr("stroke-width","2").attr("d","M 0 -4 l 0 8"),S.append("marker").attr("id","data-sd-cap").attr("orient","auto-start-reverse").attr("markerUnits","userSpaceOnUse").attr("viewBox","-5 -5 10 10").attr("refX","0").attr("refY","0").attr("markerWidth","10").attr("markerHeight","10").append("path").attr("stroke","context-stroke").attr("fill","context-stroke").attr("stroke-width","2").attr("d","M 0 -3 l 0 6");const T=S.append("linearGradient").attr("id","path-animate").attr("gradientUnits","userSpaceOnUse").attr("color-interpolation","linearRGB").attr("x1","0").attr("x2","0").attr("y1",$(this.bounds.upper)).attr("y2",$(this.bounds.lower));T.append("stop").classed("stop-0",!0).attr("offset","0%"),T.append("stop").classed("stop-100",!0).attr("offset","100%");const N=x.merge(R).attr("viewBox",`0 0 ${n} ${i}`),D=N.selectAll(".plot.density").data([{outcome:"correct",data:{meanRT:this.data.correctMeanRT,sdRT:this.data.correctSDRT},model:{meanRT:this.model.correctMeanRT,sdRT:this.model.correctSDRT,dist:this.model.correctDist},densityScale:I,densityLine:U,alignDistribution:this.alignCorrectDistribution.bind(this)},{outcome:"error",data:{meanRT:this.data.errorMeanRT,sdRT:this.data.errorSDRT},model:{meanRT:this.model.errorMeanRT,sdRT:this.model.errorSDRT,dist:this.model.errorDist},densityScale:C,densityLine:F,alignDistribution:this.alignErrorDistribution.bind(this)}]),w=x.append("g").classed("plot evidence",!0),L=D.enter().append("g").attr("class",(t=>`plot density ${t.outcome}`)),M=x.append("g").classed("plot accuracy",!0),_=N.select(".plot.evidence").attr("transform",`translate(${o}, ${a+p+d})`),O=L.merge(D).attr("transform",(t=>`translate(${o}, ${"correct"===t.outcome?a:a+p+h+2*d})`)),k=N.select(".plot.accuracy").attr("transform",`translate(${o+y+g}, ${a})`);w.append("clipPath").attr("id","clip-evidence").append("rect"),_.select("clipPath rect").attr("y",$(this.bounds.upper)).attr("height",$(this.bounds.lower)-$(this.bounds.upper)+1).attr("width",y+1);const P=w.append("g").classed("underlayer",!0),H=L.append("g").classed("underlayer",!0),V=M.append("g").classed("underlayer",!0),G=_.select(".underlayer"),z=O.select(".underlayer"),Y=k.select(".underlayer");w.append("g").classed("content",!0).append("g").classed("paths",!0);const W=L.append("g").classed("content",!0);M.append("g").classed("content",!0);const j=_.select(".content"),K=O.select(".content"),J=k.select(".content");w.append("g").classed("overlayer",!0),L.append("g").classed("overlayer",!0),M.append("g").classed("overlayer",!0);const X=_.select(".overlayer"),Z=O.select(".overlayer");P.append("rect").classed("background",!0),H.append("rect").classed("background",!0),G.select(".background").transition().duration(this.drag?0:b).ease(cubicOut).attr("y",$(this.bounds.upper)).attr("height",$(this.bounds.lower)-$(this.bounds.upper)).attr("width",y),z.select(".background").transition().duration(b).ease(cubicOut).attr("height",p).attr("width",y),H.filter((t=>"error"===t.outcome)).append("g").classed("axis time",!0);const q=z.filter((t=>"error"===t.outcome)).select(".axis.time").attr("transform",`translate(0, ${p+.25*this.rem})`);q.transition().duration(b).ease(cubicOut).call(axisBottom(f)).attr("font-size",null).attr("font-family",null).selectAll("line, path").attr("stroke",null);const tt=H.filter((t=>"error"===t.outcome)).append("text").classed("title time",!0).attr("text-anchor","middle");tt.append("tspan").classed("name",!0).text("Time (ms)"),z.filter((t=>"error"===t.outcome)).select(".title.time").transition().duration(b).ease(cubicOut).attr("transform",`translate(${y/2}, ${p+2.5*this.rem})`),P.append("g").classed("axis evidence",!0),H.append("g").attr("class",(t=>`axis density ${t.outcome}`)),V.append("g").classed("axis accuracy",!0);const et=G.select(".axis.evidence").attr("transform",`translate(${-.25*this.rem}, 0)`),rt=z.select(".axis.density").attr("transform",`translate(${-.25*this.rem}, 0)`),nt=Y.select(".axis.accuracy").attr("transform",`translate(${m+.25*this.rem}, 0)`),it=et.transition().duration(b).ease(cubicOut).call(axisLeft($)).attr("font-size",null).attr("font-family",null),at=rt.transition().duration(b).ease(cubicOut).each(((t,e,r)=>{axisLeft(t.densityScale).ticks(2)(select(r[e]))})).attr("font-size",null).attr("font-family",null),st=nt.transition().duration(b).ease(cubicOut).call(axisRight(E)).attr("font-size",null).attr("font-family",null);it.selectAll("line, path").attr("stroke",null),at.selectAll("line, path").attr("stroke",null),st.selectAll("line, path").attr("stroke",null);const ot=P.append("text").classed("title evidence",!0).attr("text-anchor","middle"),lt=H.append("text").attr("class",(t=>`title density ${t.outcome}`)).attr("text-anchor","middle"),ct=V.append("text").classed("title accuracy",!0).attr("text-anchor","middle");ot.append("tspan").classed("name",!0).text("Evidence"),lt.append("tspan").classed("name",!0).text("Density"),ct.append("tspan").classed("name",!0).text("Accuracy"),G.select(".title.evidence").transition().duration(b).ease(cubicOut).attr("transform",`translate(${-2.5*this.rem}, ${h/2})rotate(-90)`),z.select(".title.density").transition().duration(b).ease(cubicOut).attr("transform",`translate(${-2.5*this.rem}, ${p/2})rotate(-90)`),Y.select(".title.accuracy").transition().duration(b).ease(cubicOut).attr("transform",`translate(${m+2.25*this.rem}, ${c/2})rotate(90)`);const ut=j.select(".paths").selectAll(".path").data(this.data.trials.filter((t=>void 0!==t.path))),dt=local(),ht=ut.enter().append("g").classed("path",!0).attr("data-new-trial-ease-time",0).on("pointerenter",((t,e)=>{if(!this.drag){select(t.currentTarget).classed("highlight",!0).raise();const r=X.append("g").classed(`rt-label ${e.outcome}`,!0),n=r.append("rect"),i=r.append("text").text(`RT = ${e.rt.toFixed()}`).attr("x",f(e.rt)).attr("y","correct"===e.outcome?$(this.bounds.upper)-.25*this.rem:$(this.bounds.lower)+.125*this.rem).node().getBBox();n.attr("x",i.x-.125*this.rem).attr("y",i.y+.125*this.rem).attr("width",i.width+.25*this.rem).attr("height",i.height-.25*this.rem),dt.set(t.currentTarget,r)}})).on("pointerout",((t,e)=>{this.drag||(select(t.currentTarget).classed("highlight",!1).lower(),t.currentTarget.parentNode.insertBefore(t.currentTarget,t.currentTarget.parentNode.children[e.index]),dt.get(t.currentTarget).remove())}));ht.append("path").classed("curve",!0).attr("clip-path","url(#clip-evidence)").attr("pathLength",1).attr("stroke-dashoffset",1);const pt=ht.merge(ut).attr("class",(t=>`path ${t.outcome}`));pt.select(".curve").transition().duration(this.drag?0:b).ease(cubicOut).attr("stroke",(t=>this.getComputedStyleValue(`---color-${t.outcome}`))).attrTween("d",((t,e,r)=>{const n=r[e],i=interpolate$1(void 0!==n.a?n.a:this.a,this.a),a=interpolate$1(void 0!==n.z?n.z:this.z,this.z),s=interpolate$1(void 0!==n.v?n.v:this.v,this.v),o=interpolate$1(void 0!==n.t0?n.t0:this.t0,this.t0);return e=>{n.a=i(e),n.z=a(e),n.v=s(e),n.t0=o(e);const r=this.alignPath(t.seed,n.a,n.z,n.v,n.t0);return v(r)}}));const gt=pt.filter((t=>t.animate&&!this.paused));if(!gt.empty()){const t=gt.attr("data-new-trial-ease-time"),e=e=>linear().domain([0,1]).range([t,1])(e),r=e=>r=>linear().domain([e(t),1]).range([0,1])(e(r));gt.classed("animate",!0).select(".curve").attr("stroke-dasharray",1),gt.transition("new").duration((e=>Math.floor(1.5*e.rt*(1-t)))).ease(e).attr("data-new-trial-ease-time",1).select(".curve").attrTween("stroke-dashoffset",((t,e,n)=>{const i=interpolate$1(n[e].getAttribute("stroke-dashoffset"),0);return t=>i(r(linear$1)(t))})).on("end",((t,e,r)=>{select(r[e].parentElement).classed("animate",!1).attr("data-new-trial-ease-time",null),t.animate=!1,this.animate=!1,this.alignState(),this.requestUpdate(),this.dispatchEvent(new CustomEvent("accumulable-response",{detail:{outcome:t.outcome,data:this.data,model:this.model},bubbles:!0}))}))}const yt=pt.filter((t=>t.animate&&this.paused));if(!yt.empty()){const t=yt.attr("data-new-trial-ease-time");yt.classed("animate",!0).select(".curve").attr("stroke-dasharray",1).attr("stroke-dashoffset",(()=>interpolate$1(1,0)(linear$1(t))))}pt.filter((t=>!t.animate)).attr("data-new-trial-ease-time",null),ut.exit().remove();const mt=W.append("g").attr("class",(t=>`dist ${t.outcome}`));mt.append("path").classed("curve",!0),K.select(".dist").select(".curve").transition().duration(this.drag?0:b).ease(cubicOut).attrTween("d",((t,e,r)=>{const n=r[e],i=interpolate$1(void 0!==n.a?n.a:this.a,this.a),a=interpolate$1(void 0!==n.z?n.z:this.z,this.z),s=interpolate$1(void 0!==n.v?n.v:this.v,this.v),o=interpolate$1(void 0!==n.t0?n.t0:this.t0,this.t0);return e=>{n.a=i(e),n.z=a(e),n.v=s(e),n.t0=o(e);const r=t.alignDistribution(n.a,n.z,n.v,n.t0);return t.densityLine(r)}}));const bt=j.selectAll(".rt").data(this.data.trials),ft=bt.enter().append("g");ft.append("line").classed("mark",!0).attr("x1",(t=>f(t.rt))).attr("x2",(t=>f(t.rt))).attr("y1",(t=>"correct"===t.outcome?$(1)-.125*this.rem:$(-1)+.125*this.rem)).attr("y2",(t=>"correct"===t.outcome?$(1)-.675*this.rem:$(-1)+.675*this.rem));const $t=ft.merge(bt).attr("class",(t=>`rt ${t.outcome}`));$t.filter((t=>!t.animate)).select(".mark").transition().duration(this.drag?0:b).ease(cubicOut).attr("stroke",(t=>this.getComputedStyleValue(`---color-${t.outcome}`))).attr("x1",(t=>f(t.rt))).attr("x2",(t=>f(t.rt))).attr("y1",(t=>"correct"===t.outcome?$(1)-.125*this.rem:$(-1)+.125*this.rem)).attr("y2",(t=>"correct"===t.outcome?$(1)-.675*this.rem:$(-1)+.675*this.rem)),bt.exit().remove();const It=J.selectAll(".accuracy.model").data([this.model.accuracy,1-this.model.accuracy]),Ct=It.enter().append("g").attr("class",((t,e)=>"accuracy model "+(0===e?"correct":"error")));Ct.append("rect").classed("bar",!0).attr("x",0),Ct.merge(It).select("rect").transition().duration(this.drag?0:b).ease(cubicOut).attr("y",((t,e)=>E(0===e?0:1-t))).attr("width",m).attr("height",(t=>E(t))),It.exit().remove();const Et=J.selectAll(".accuracy.data").data(Number.isNaN(this.data.accuracy)?[]:[this.data.accuracy]),Bt=Et.enter().append("g").classed("accuracy data",!0);Bt.append("line").classed("mark",!0);const At=Bt.merge(Et).attr("class",(t=>"accuracy data "+(t<this.model.accuracy.correct?"correct":"error")));At.select(".mark").transition().duration(this.drag?0:b).ease(cubicOut).attr("x1",0+.25*this.rem).attr("x2",m-.25*this.rem).attr("y1",(t=>E(t)-1)).attr("y2",(t=>E(t)-1)),Et.exit().remove();const Qt=X.selectAll(".boundary").data([{bound:"upper",value:this.bounds.upper},{bound:"lower",value:this.bounds.lower}]),vt=Qt.enter().append("g").attr("class",((t,e)=>"boundary "+(0===e?"correct":"error")));vt.append("line").classed("line",!0);const Ut=vt.merge(Qt).attr("tabindex",this.interactive?0:null).classed("interactive",this.interactive).on("keydown",this.interactive?(t,e)=>{if(["ArrowUp","ArrowDown"].includes(t.key)){let r=this.a;switch(t.key){case"ArrowUp":r+="upper"===e.bound?t.shiftKey?.01:.1:t.shiftKey?-.01:-.1;break;case"ArrowDown":r+="upper"===e.bound?t.shiftKey?-.01:-.1:t.shiftKey?.01:.1}r=r<.01?.01:r>2*this.scale.evidence.max?2*this.scale.evidence.max:r,this.a=r,this.alignState(),this.dispatchEvent(new CustomEvent("ddm-model-a",{detail:{a:this.a},bubbles:!0})),t.preventDefault()}}:null);(this.firstUpdate||t.has("interactive"))&&(this.interactive?Ut.call(Q):Ut.on(".drag",null)),Ut.select(".line").transition().duration(this.drag?0:b).ease(cubicOut).attr("x1",f(this.scale.time.min)).attr("x2",f(this.scale.time.max)).attr("y1",(t=>$(t.value))).attr("y2",(t=>$(t.value))),Qt.exit().remove();const Ft=X.selectAll(".drift").data([{v:this.v,t0:this.t0,startingPoint:this.startingPoint}]),Rt=Ft.enter().append("g").classed("drift",!0);Rt.append("line").classed("line",!0),Rt.append("path").classed("arrow",!0);const xt=Rt.merge(Ft).attr("tabindex",this.interactive?0:null).classed("interactive",this.interactive).on("keydown",this.interactive?t=>{if(["ArrowUp","ArrowDown"].includes(t.key)){let e=this.v;switch(t.key){case"ArrowUp":e+=t.shiftKey?.01:.1;break;case"ArrowDown":e-=t.shiftKey?.01:.1}e=e<.01?.01:e>5?5:e,this.v=e,this.alignState(),this.dispatchEvent(new CustomEvent("ddm-model-v",{detail:{v:this.v},bubbles:!0})),t.preventDefault()}}:null);(this.firstUpdate||t.has("interactive"))&&(this.interactive?xt.call(A):xt.on(".drag",null));const St=($(0)-$(1))/(f(1)-f(0));xt.transition().duration(this.drag?0:b).ease(cubicOut).attr("transform",(t=>`translate(${f(t.t0)}, ${$(t.startingPoint)})\n          rotate(${-Math.atan(t.v/1e3*St)*(180/Math.PI)})`)),xt.select(".line").attr("x2",f(200)),xt.select(".arrow").attr("d",`\n        M ${f(200)-.5*this.rem},${.5*-this.rem}\n        l ${.5*this.rem},${.5*this.rem}\n        l ${.5*-this.rem},${.5*this.rem}\n      `),Ft.exit().remove();const Tt=X.selectAll(".t0z").data([{t0:this.t0,startingPoint:this.startingPoint}]),Nt=Tt.enter().append("g").classed("t0z",!0);Nt.append("line").classed("line",!0),Nt.append("circle").classed("point",!0);const Dt=Nt.merge(Tt).attr("tabindex",this.interactive?0:null).classed("interactive",this.interactive).on("keydown",this.interactive?t=>{if(["ArrowUp","ArrowDown"].includes(t.key)){let e=this.z;switch(t.key){case"ArrowUp":e+=t.shiftKey?.01:.1;break;case"ArrowDown":e-=t.shiftKey?.01:.1}e=e<.01?.01:e>.99?.99:e,this.z=e,this.alignState(),this.dispatchEvent(new CustomEvent("ddm-model-z",{detail:{z:this.z},bubbles:!0})),t.preventDefault()}if(["ArrowLeft","ArrowRight"].includes(t.key)){let e=this.t0;switch(t.key){case"ArrowRight":e+=t.shiftKey?1:10;break;case"ArrowLeft":e-=t.shiftKey?1:10}e=e<0?0:e>500?500:e,this.t0=e,this.alignState(),this.dispatchEvent(new CustomEvent("ddm-model-t0",{detail:{t0:this.t0},bubbles:!0})),t.preventDefault()}}:null);(this.firstUpdate||t.has("interactive"))&&(this.interactive?Dt.call(B):Dt.on(".drag",null)),Dt.select(".line").transition().duration(this.drag?0:b).ease(cubicOut).attr("x1",f(0)).attr("x2",(t=>f(t.t0))).attr("y1",(t=>$(t.startingPoint))).attr("y2",(t=>$(t.startingPoint))),Dt.select(".point").transition().duration(this.drag?0:b).ease(cubicOut).attr("cx",(t=>f(t.t0))).attr("cy",(t=>$(t.startingPoint))),Tt.exit().remove();const wt=X.selectAll(".measure.a").data(this.measures?[this.a]:[]),Lt=wt.enter().append("g").classed("measure a",!0);Lt.append("line").classed("line",!0).attr("marker-start","url(#measure-arrow)").attr("marker-end","url(#measure-arrow)");const Mt=Lt.append("text").classed("label",!0);Mt.append("tspan").classed("a math-var",!0).text("a"),Mt.append("tspan").classed("equals",!0).text(" = "),Mt.append("tspan").classed("value",!0);const _t=Lt.merge(wt);_t.select(".line").transition().duration(this.drag?0:b).ease(cubicOut).attr("x1",f(this.scale.time.max)-.75*this.rem).attr("y1",$(this.bounds.upper)+2).attr("x2",f(this.scale.time.max)-.75*this.rem).attr("y2",$(this.bounds.lower)-2);_t.select(".label").transition().duration(this.drag?0:b).ease(cubicOut).attr("x",f(this.scale.time.max)).attr("y",$(this.bounds.upper)-.25*this.rem).select(".value").text(format(".2f")(this.a)),wt.exit().remove();const Ot=X.selectAll(".measure.z").data(this.measures?[this.z]:[]),kt=Ot.enter().append("g").classed("measure z",!0);kt.append("line").classed("line",!0).attr("marker-start","url(#measure-arrow)").attr("marker-end","url(#measure-arrow)");const Pt=kt.append("text").classed("label",!0);Pt.append("tspan").classed("z math-var",!0).text("z"),Pt.append("tspan").classed("equals",!0).text(" = "),Pt.append("tspan").classed("value",!0);const Ht=kt.merge(Ot);Ht.select(".line").transition().duration(this.drag?0:b).ease(cubicOut).attr("x1",f(this.scale.time.min)+.75*this.rem).attr("y1",$(this.startingPoint)+2).attr("x2",f(this.scale.time.min)+.75*this.rem).attr("y2",$(this.bounds.lower)-2);Ht.select(".label").transition().duration(this.drag?0:b).ease(cubicOut).attr("x",f(this.scale.time.min)).attr("y",$(this.bounds.lower)+.125*this.rem).select(".value").text(format(".0%")(this.z)),Ot.exit().remove();const Vt=X.selectAll(".measure.v").data(this.measures?[this.v]:[]),Gt=Vt.enter().append("g").classed("measure v",!0);Gt.append("path").classed("line",!0).attr("marker-start","url(#measure-arrow)").attr("marker-end","url(#measure-arrow)");const zt=Gt.append("text").classed("label",!0);zt.append("tspan").classed("v math-var",!0).text("v"),zt.append("tspan").classed("equals",!0).text(" = "),zt.append("tspan").classed("value",!0);const Yt=Math.atan(this.v/1e3*St),Wt=f(200)-f(0)+.75*this.rem,jt=Math.cos(Yt)*Wt,Kt=Math.sin(Yt)*Wt,Jt=Gt.merge(Vt);Jt.select(".line").transition().duration(this.drag?0:b).ease(cubicOut).attr("d",`\n        M ${f(this.t0+200)+.75*this.rem}, ${$(this.startingPoint)}\n        A ${f(200)-f(0)} ${f(200)-f(0)} 0 0 0 ${f(this.t0)+jt} ${$(this.startingPoint)-Kt}\n      `);Jt.select(".label").transition().duration(this.drag?0:b).ease(cubicOut).attr("x",f(this.t0+200)+.5*this.rem).attr("y",$(this.bounds.upper)-.25*this.rem).select(".value").text(format(".2f")(this.v)),Vt.exit().remove();const Xt=X.selectAll(".measure.t0").data(this.measures?[this.t0]:[]),Zt=Xt.enter().append("g").classed("measure t0",!0);Zt.append("line").classed("line",!0).attr("marker-start","url(#measure-arrow)").attr("marker-end","url(#measure-arrow)");const qt=Zt.append("text").classed("label",!0);qt.append("tspan").classed("t0 math-var",!0).text("t₀"),qt.append("tspan").classed("equals",!0).text(" = "),qt.append("tspan").classed("value",!0);const te=Zt.merge(Xt);te.select(".line").transition().duration(this.drag?0:b).ease(cubicOut).attr("x1",f(0)+2).attr("y1",$(this.startingPoint)-.75*this.rem).attr("x2",f(this.t0)-2).attr("y2",$(this.startingPoint)-.75*this.rem);te.select(".label").transition().duration(this.drag?0:b).ease(cubicOut).attr("x",f(this.t0)+.25*this.rem).attr("y",$(this.bounds.upper)-.25*this.rem).select(".value").text(format("d")(this.t0)),Xt.exit().remove();const ee=Z.selectAll(".model.mean").data((t=>this.means?[t]:[])),re=ee.enter().append("g").attr("class",(t=>`model mean ${t.outcome}`));re.append("line").classed("indicator",!0);re.merge(ee).select(".indicator").transition().duration(this.drag?0:b).ease(cubicOut).attr("x1",(t=>f(t.model.meanRT))).attr("x2",(t=>f(t.model.meanRT))).attr("y1",(t=>t.densityScale(this.scale.density.min))).attr("y2",(t=>t.densityScale(this.scale.density.max))),ee.exit().remove();const ne=Z.selectAll(".data.mean").data((t=>this.means&&!Number.isNaN(t.data.meanRT)?[t]:[])),ie=ne.enter().append("g").attr("class",(t=>`data mean ${t.outcome}`));ie.append("line").classed("indicator",!0).attr("y1",(t=>t.densityScale(0)+("correct"===t.outcome?.125:-.125)*this.rem)).attr("y2",(t=>t.densityScale(0)+("correct"===t.outcome?.675:-.675)*this.rem));ie.merge(ne).select(".indicator").transition().duration(this.drag?0:b).ease(cubicOut).attr("x1",(t=>f(t.data.meanRT))).attr("x2",(t=>f(t.data.meanRT))).attr("y1",(t=>t.densityScale(0)+("correct"===t.outcome?.125:-.125)*this.rem)).attr("y2",(t=>t.densityScale(0)+("correct"===t.outcome?.675:-.675)*this.rem)),ne.exit().select(".indicator").transition().duration(this.drag?0:b).ease(cubicOut).attr("x1",0).attr("x2",0).on("end",((t,e,r)=>{select(r[e].parentElement).remove()}));const ae=Z.selectAll(".model.sd").data((t=>this.sds?[t]:[])),se=ae.enter().append("g").attr("class",(t=>`model sd ${t.outcome}`));se.append("line").classed("indicator",!0).attr("marker-start","url(#model-sd-cap)").attr("marker-end","url(#model-sd-cap)");se.merge(ae).select(".indicator").transition().duration(this.drag?0:b).ease(cubicOut).attr("x1",(t=>f(t.model.meanRT-t.model.sdRT/2))).attr("x2",(t=>f(t.model.meanRT+t.model.sdRT/2))).attr("y1",(t=>t.densityScale(5))).attr("y2",(t=>t.densityScale(5))),ae.exit().remove();const oe=Z.selectAll(".data.sd").data((t=>!this.sds||Number.isNaN(t.data.meanRT)||Number.isNaN(t.data.sdRT)?[]:[t])),le=oe.enter().append("g").attr("class",(t=>`data sd ${t.outcome}`));le.append("line").classed("indicator",!0).attr("marker-start","url(#data-sd-cap)").attr("marker-end","url(#data-sd-cap)").attr("y1",(t=>t.densityScale(0)+("correct"===t.outcome?.375:-.375)*this.rem)).attr("y2",(t=>t.densityScale(0)+("correct"===t.outcome?.375:-.375)*this.rem));le.merge(oe).select(".indicator").transition().duration(this.drag?0:b).ease(cubicOut).attr("x1",(t=>f(t.data.meanRT-t.data.sdRT/2))).attr("x2",(t=>f(t.data.meanRT+t.data.sdRT/2))).attr("y1",(t=>t.densityScale(0)+("correct"===t.outcome?.375:-.375)*this.rem)).attr("y2",(t=>t.densityScale(0)+("correct"===t.outcome?.375:-.375)*this.rem)),oe.exit().select(".indicator").transition().duration(this.drag?0:b).ease(cubicOut).attr("x1",0).attr("x2",0).on("end",((t,e,r)=>{select(r[e].parentElement).remove()})),this.firstUpdate=!1}}customElements.define("ddm-model",DDMModel);class RDK2AFCTask extends(DecidablesMixinResizeable(AccumulableElement)){static get properties(){return{coherence:{attribute:"coherence",type:Number,reflect:!0},count:{attribute:"count",type:Number,reflect:!0},probability:{attribute:"probability",type:Number,reflect:!0},duration:{attribute:"duration",type:Number,reflect:!0},wait:{attribute:"wait",type:Number,reflect:!0},iti:{attribute:"iti",type:Number,reflect:!0},trials:{attribute:"trials",type:Number,reflect:!0},running:{attribute:"running",type:Boolean,reflect:!0},lifetime:{attribute:!1,type:Number,reflect:!1},speed:{attribute:!1,type:Number,reflect:!1}}}constructor(){super(),this.coherence=.5,this.count=100,this.probability=.5,this.duration=2e3,this.wait=2e3,this.iti=2e3,this.trials=5,this.running=!1,this.lifetime=400,this.speed=50,this.firstUpdate=!0,this.COHERENT=0,this.RANDOM=1,this.dots=[[],[]],this.trial=0,this.states=["resetted","iti","stimulus","wait","ended"],this.state="resetted",this.baseTime=0,this.pauseTime=0,this.startTime=0,this.lastTime=0,this.LEFT=180,this.RIGHT=0,this.currentDirection=void 0,this.signals=["left","right"],this.signal=void 0,this.runner=void 0,this.xScale=void 0,this.yScale=void 0}static get styles(){return[super.styles,i$2`
        :host {
          display: inline-block;
      ...
      </div>`}}customElements.define("ddm-equation-azvt02m",DDMEquationAZVT02M);class DDMExample extends AccumulableElement{static get styles(){return[super.styles,i$2`
        :host {
          ---border: var(--border, 1px solid var(---color-border));
          display: inline-block;
