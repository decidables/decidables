...
var slice$3 = uncurryThis$h(ArrayBufferPrototype.slice);
...
var NAME$4, Constructor, Prototype;
...
for (NAME$4 in TypedArrayConstructorsList) {
  Constructor = globalThis$7[NAME$4];
  Prototype = Constructor && Constructor.prototype;
  if (Prototype) enforceInternalState(Prototype)[TYPED_ARRAY_CONSTRUCTOR] = Constructor;else NATIVE_ARRAY_BUFFER_VIEWS = false;
}
for (NAME$4 in BigIntArrayConstructorsList) {
  Constructor = globalThis$7[NAME$4];
  Prototype = Constructor && Constructor.prototype;
...
if (!NATIVE_ARRAY_BUFFER_VIEWS || !isCallable$5(TypedArray) || TypedArray === Function.prototype) {
  ...
  if (NATIVE_ARRAY_BUFFER_VIEWS) for (NAME$4 in TypedArrayConstructorsList) {
    if (globalThis$7[NAME$4]) setPrototypeOf$1(globalThis$7[NAME$4], TypedArray);
  }
}
if (!NATIVE_ARRAY_BUFFER_VIEWS || !TypedArrayPrototype || TypedArrayPrototype === ObjectPrototype) {
  TypedArrayPrototype = TypedArray.prototype;
  if (NATIVE_ARRAY_BUFFER_VIEWS) for (NAME$4 in TypedArrayConstructorsList) {
    if (globalThis$7[NAME$4]) setPrototypeOf$1(globalThis$7[NAME$4].prototype, TypedArrayPrototype);
  }
}
...
if (DESCRIPTORS$3 && !hasOwn$4(TypedArrayPrototype, TO_STRING_TAG)) {
  ...
  for (NAME$4 in TypedArrayConstructorsList) if (globalThis$7[NAME$4]) {
    createNonEnumerableProperty$1(globalThis$7[NAME$4], TYPED_ARRAY_TAG, NAME$4);
  }
}
...
EventHandler.on(document, EVENT_CLICK_DATA_API$2, SELECTOR_DATA_TOGGLE$1, event => {
...
*** Add handles to lines?
  ...
  Styles:
    ??
*/
class CPTProbability extends DecidablesMixinResizeable(ProspectableElement) {
  static get properties() {
    return {
      p: {
        attribute: 'probability',
  ...
  update(changedProperties) {
    ...
    const curveDrag = drag().subject((event, datum) => {
      ...
      const g = event.subject.p > event.subject.w ? pDiff > wDiff ? event.subject.g - distance : event.subject.g + distance : pDiff > wDiff ? event.subject.g + distance : event.subject.g - distance;
    ...
    // Clippath
    //  ENTER
    plotEnter.append('clipPath').attr('id', 'clip-cpt-value').append('rect');
    //  MERGE
    plotMerge.select('clipPath rect').attr('height', height + 1).attr('width', width + 1);
    ...
    // X Axis/Scale
    //  ENTER
    underlayerEnter.append('g').classed('scale-x', true);
    //  MERGE
    ...
    // Y Axis/Scale
    //  ENTER
    underlayerEnter.append('g').classed('scale-y', true);
    // MERGE
    ...
    .attr('x1', xScale(0)).attr('y1', yScale(0)).attr('x2', xScale(1)).attr('y2', yScale(1));
    ...
    //  ENTER
    const lineEnter = lineUpdate.enter().append('g').classed('lines', true);
    //  ENTER - All
    ...
    //  ENTER
    const curveEnter = curveUpdate.enter().append('path').classed('curve', true).attr('clip-path', 'url(#clip-cpt-value)');
    //  MERGE
    ...
    pointMerge.filter(datum => {
      return (this.firstUpdate || changedProperties.has('interactive')) && !this.interactive || datum.trial;
    }).attr('tabindex', null).classed('interactive', false).on('drag', null).on('keydown', null);
    // All points
...
*** Add handles to lines?
  ...
  Styles:
    ??
*/
class CPTValue extends DecidablesMixinResizeable(ProspectableElement) {
  static get properties() {
    return {
      x: {
        attribute: 'value',
  ...
  update(changedProperties) {
    ...
    // X Scale
    const xScale = linear().domain([-20, domainScale]).range([0, width]);
    this.xScale = xScale;

    // Y Scale
    const yScale = linear().domain([domainScale, -20]).range([0, height]);
    this.yScale = yScale;
    ...
    const pointDrag = drag().subject((event, datum) => {
      ...
      datum.x = x < -20 ? -20 : x > domainScale ? domainScale : x;
      if (datum.name === 'default') {
        this.x = datum.x;
      }
      this.alignState();
    ...
    underlayerEnter.append('g').classed('scale-x', true);
    //  MERGE
    const scaleXMerge = underlayerMerge.select('.scale-x').attr('transform', `translate(0, ${yScale(-20)})`);
    const scaleXTransition = scaleXMerge.transition().duration(transitionDuration * 2) // Extra long transition!
    .ease(cubicOut).call(axisBottom(xScale)).attr('font-size', null).attr('font-family', null);
    scaleXTransition.selectAll('line, path').attr('stroke', null);
    ...
    // X Axis
    //  ENTER
    underlayerEnter.append('line').classed('axis-x', true);
    //  MERGE
    underlayerMerge.select('.axis-x').transition().duration(transitionDuration).ease(cubicOut).attr('x1', xScale(-20)).attr('x2', xScale(domainScale)).attr('y1', yScale(0)).attr('y2', yScale(0));
    ...
    // Y Axis
    //  ENTER
    underlayerEnter.append('line').classed('axis-y', true);
    //  MERGE
    underlayerMerge.select('.axis-y').transition().duration(transitionDuration).ease(cubicOut).attr('x1', xScale(0)).attr('x2', xScale(0)).attr('y1', yScale(domainScale)).attr('y2', yScale(-20));
    ...
    //  MERGE
    underlayerMerge.select('.diagonal').transition().duration(transitionDuration * 2) // Extra long transition!
    .ease(cubicOut).attr('x1', xScale(-20)).attr('y1', yScale(-20)).attr('x2', xScale(domainScale)).attr('y2', yScale(domainScale));
    ...
    lineEnter.filter(datum => {
      ...
      selection.select('.line-x.below').attr('x1', xScale(datum.x)).attr('x2', xScale(datum.x)).attr('y1', yScale(-20)).attr('y2', yScale(-20));
    ...
    if (!lineMergeActive.empty()) {
      ...
      lineMergeActive.transition('new-1').duration(() => {
        ...
        return time => {
          ...
          selection.select('.line-x.below').attr('x1', xScale(element.x)).attr('x2', xScale(element.x)).attr('y1', yScale(interpolateBelow(scaleOutGenerator1(cubicIn)(time)))).attr('y2', yScale(-20));
        };
      }).transition('new-2').duration(() => {
        return Math.floor(transitionDuration * (1 - easeTime2));
      }).ease(scaleIn2).attr('data-animating-ease-time-2', 1).tween('animating', (datum, index, elements) => {
        ...
        const interpolateA = interpolate$1(element.a !== undefined ? element.a : this.getFunction(datum.function).a, this.getFunction(datum.function).a);
        const interpolateL = interpolate$1(element.l !== undefined ? element.l : this.getFunction(datum.function).l, this.getFunction(datum.function).l);
        const interpolateBefore = interpolate$1(xScale.invert(selection.select('.line-v.before').attr('x1')), -20);
    ...
    if (!lineMergePaused.empty()) {
      ...
      lineMergePaused.transition().duration(this.drag ? 0 : this.firstUpdate ? transitionDuration * 2 : transitionDuration).ease(cubicOut).tween('paused', (datum, index, elements) => {
        ...
        const interpolateBelow = interpolate$1(-20, datum.v);
        const interpolateBefore = interpolate$1(datum.x, -20);
        ...
        return time => {
          ...
          selection.select('.line-x.below').attr('x1', xScale(element.x)).attr('x2', xScale(element.x)).attr('y1', yScale(interpolateBelow(cubicIn(easeTime1)))).attr('y2', yScale(-20));
    ...
    lineMerge.filter(datum => {
      ...
      return time => {
        ...
        selection.select('.line-x.below').attr('x1', xScale(element.x)).attr('x2', xScale(element.x)).attr('y1', yScale(element.v)).attr('y2', yScale(-20));
        selection.select('.line-v.before').attr('x1', xScale(-20)).attr('x2', xScale(element.x)).attr('y1', yScale(element.v)).attr('y2', yScale(element.v));
    ...
    //  ENTER
    const curvePEnter = curvePUpdate.enter().append('path').classed('curve-p', true).attr('clip-path', 'url(#clip-cpt-value)');
    //  MERGE
    ...
    //  ENTER
    const curveNEnter = curveNUpdate.enter().append('path').classed('curve-n', true).attr('clip-path', 'url(#clip-cpt-value)');
    //  MERGE
    ...
    pointMerge.filter(datum => {
      ...
      if (['ArrowUp', 'ArrowDown', 'ArrowRight', 'ArrowLeft'].includes(event.key)) {
        ...
        // Clamp x to visible plot
        x = x < -20 ? -20 : x > domainScale ? domainScale : x;
        if (x !== datum.x) {
          datum.x = x;
          if (datum.name === 'default') {
            this.x = datum.x;
...
*/
class RiskyOption extends DecidablesMixinResizeable(ProspectableElement) {
  static get styles() {
    return [super.styles, i$3`
        :host {
  ...
  update(changedProperties) {
    ...
    // Pie
    //  ENTER
    const pieEnter = svgEnter.append('g').classed('pie', true);
    //  MERGE
    const pieMerge = svgMerge.select('.pie').attr('transform', `translate(${width / 2}, ${height / 2})`);

    // Outline
    //  ENTER
    pieEnter.append('circle').classed('outline', true);
    //  MERGE
    pieMerge.select('.outline').attr('r', Math.min(width, height) / 2 - 1);

    // Arcs
    //  DATA-JOIN
    const arcUpdate = pieMerge.selectAll('.arc').data(arcs);
    //  ENTER
    ...
    }).attr('d', arc().innerRadius(0).outerRadius(Math.min(width, height) / 2 - 1));
    //  EXIT
...
class RiskyTask extends ProspectableElement {
  ...
  constructor() {
    ...
    this.range.xs.values = range(this.range.xs.start, this.range.xs.stop + 0.01, this.range.xs.step);
