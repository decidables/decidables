...
- `yarn workspace @decidable/<library|site> lint`
  - Lints a single library or site
- `yarn workspace @decidable/<library|site> gulp lint<type>`
...
- `yarn workspace @decidable/<site> compile`
  - Compiles a single site from `src/` to `local/`
- `yarn workspace @decidable/<site> gulp compile<type>`
  - Compiles `Config`, `Fonts`, `Markdown`, `Scripts`, or `Styles` in a single site from `src/` to `local/`
...
- `yarn workspace @decidable/<library|site> build`
...
- `yarn workspace @decidable/<library|site> gulp build<type>`
  - Builds `Scripts` in a single library from `src/` to `lib/`
  - Builds `Config`, `Fonts`, `Markdown`, `Scripts`, or `Styles` in a single site from `local/` to `dist/`
...
- `yarn serve`
  - Serves all sites mounted on `local/` and `dist/` (e.g. <http://localhost:8000/local/detectable> or <http://localhost:8000/dist/detectable>)
